  ...
  const handleRegistreren = async (e) => {
    ...
    try {
      const { error } = await supabase.auth.signUp({
        email, password: wachtwoord,
        options: {
          data: { naam, account_type: accountType },
    ...
    <div style={{ minHeight: "100vh", background: "#FAF8F4", fontFamily: "Helvetica, Arial, sans-serif", overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: 440, margin: "40px auto", padding: "0 16px 80px" }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "clamp(20px, 5vw, 36px) clamp(16px, 5vw, 32px)", boxShadow: "0 4px 24px rgba(11,42,82,0.08)" }}>
          ...
          {/* Fout/succes melding */}
          {fout && <div style={{ background: "#fdf0f0", border: "1px solid #e5b0b0", color: "#8B2020", padding: "11px 14px", borderRadius: 4, fontSize: 14, marginBottom: 16 }}>{fout}</div>}
          ...
          {modus === "registreren" && (
            <form onSubmit={handleRegistreren} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* Accounttype */}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: NAVY, display: "block", marginBottom: 10 }}>Accounttype <span style={{ color: GOLD }}>*</span></label>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[
                    { value: "particulier", label: "Particulier", sub: "1 woning plaatsen, €195 eenmalig", kleur: NAVY },
                    { value: "makelaar_starter", label: "Makelaar / Starter", sub: "Tot 5 woningen, €250/maand", kleur: "#1A4A7A" },
                    { value: "makelaar_pro", label: "Makelaar / Pro", sub: "Onbeperkt woningen, €450/maand", kleur: GOLD },
                  ].map((opt) => (
                    <label
                      key={opt.value}
                      onClick={() => setAccountType(opt.value)}
                      style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", borderRadius: 6, border: `2px solid ${accountType === opt.value ? opt.kleur : BORDER}`, background: accountType === opt.value ? "#FAF8F4" : "#fff", cursor: "pointer", transition: "all 0.15s" }}
                    >
                      <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${accountType === opt.value ? opt.kleur : "#ccc"}`, background: accountType === opt.value ? opt.kleur : "#fff", flexShrink: 0, marginTop: 2, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {accountType === opt.value && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff" }} />}
                      </div>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14, color: accountType === opt.value ? opt.kleur : NAVY }}>{opt.label}</div>
                        <div style={{ fontSize: 12.5, color: "#666", marginTop: 2 }}>{opt.sub}</div>
                      </div>
                    </label>
                  ))}
                </div>
                <div style={{ fontSize: 12, color: "#888", marginTop: 8 }}>Account aanmaken is altijd gratis. Kosten gelden pas bij plaatsing.</div>
              </div>
