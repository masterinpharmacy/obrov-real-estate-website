...
export default function AuthPagina() {
    ...
    try {
      ...
      setFout(
        msg.includes("Invalid login credentials") || msg.includes("invalid_credentials")
          ...
          ? "Bevestig eerst uw e-mailadres via de mail die wij hebben gestuurd."
  ...
  const handleRegistreren = async (e) => {
    ...
    if (!supabaseConfigured || !supabase) {
      setFout("De database is nog niet geconfigureerd. Neem contact op via info@obrovrealestate.nl.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const msg = err?.message || err?.error_description || JSON.stringify(err) || "Er ging iets mis.";
      setFout(
    ...
    <div style={{ minHeight: "100vh", background: "#FAF8F4", fontFamily: "Helvetica, Arial, sans-serif", overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: 440, margin: "40px auto", padding: "0 16px 80px" }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "clamp(20px, 5vw, 36px) clamp(16px, 5vw, 32px)", boxShadow: "0 4px 24px rgba(11,42,82,0.08)" }}>
          ...
          {/* Fout/succes melding */}
          {fout && <div style={{ background: "#fdf0f0", border: "1px solid #e5b0b0", color: "#8B2020", padding: "11px 14px", borderRadius: 4, fontSize: 14, marginBottom: 16 }}>
            {typeof fout === "string" ? fout : "Er ging iets mis. Probeer het opnieuw."}
          </div>}
