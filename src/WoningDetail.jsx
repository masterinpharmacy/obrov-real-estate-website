...
// Escapet tekst die in de kaart-HTML wordt gezet, zodat een adres met
// een apostrof of een < de iframe niet breekt.
...
// Bouwt een zelfstandige Leaflet-kaart met OpenStreetMap-tiles.
// Zelfde aanpak en zelfde pin als de kaartweergave op de aanbodpagina,
// zodat de twee kaarten er hetzelfde uitzien. Geen API-key nodig.
function detailKaartHtml(woning) {
  const lat = Number(woning.lat);
  const lng = Number(woning.lng);
  const plaats = escapeHtml(
    [woning.stad, woning.regio && woning.regio !== woning.stad ? woning.regio : null]
      .filter(Boolean)
      .join(", ")
  );

  const pinSvg =
    '<div style="width:22px;height:30px"><svg viewBox=\\"0 0 22 30\\" xmlns=\\"http://www.w3.org/2000/svg\\" style=\\"width:22px;height:30px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.3))\\"><path d=\\"M11 0C4.925 0 0 4.925 0 11c0 7.7 11 19 11 19s11-11.3 11-19C22 4.925 17.075 0 11 0z\\" fill=\\"#0B2A52\\"/><circle cx=\\"11\\" cy=\\"11\\" r=\\"4.5\\" fill=\\"#AC9362\\"/></svg></div>';
...
<style>
body{margin:0}
#map{height:100vh;background:#E8E4DA}
.leaflet-control-attribution{font-size:10px}
</style></head><body>
<div id="map"></div><script>
var map = L.map('map', { scrollWheelZoom: false }).setView([${lat}, ${lng}], 14);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);
L.marker([${lat},${lng}],{icon:L.divIcon({className:'',html:'${pinSvg}',iconAnchor:[11,30]})})
  .addTo(map)
  .bindPopup('${plaats}');
</script></body></html>`;
}
          ...
          <div>
            ...
            {woning.lat && woning.lng && (
              <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ height: 320, borderRadius: 6, overflow: "hidden", border: `1px solid ${BORDER}` }}>
                  <iframe
                    srcDoc={detailKaartHtml(woning)}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                  <div style={{ fontSize: 12.5, color: "#6B6B6B" }}>
                    {woning.adres ? `${woning.adres}, ` : ""}{woning.stad}{woning.regio && woning.regio !== woning.stad ? `, ${woning.regio}` : ""}
                  </div>
                  <a
                    href={`https://www.google.com/maps?q=${woning.lat},${woning.lng}`}
                    target="_blank"
                    rel="noreferrer"
