...
import { getLang, FAQ_VERTALINGEN, TAAL_NAMEN } from "./lang.js";
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 800,
          system: `You are a friendly customer service representative for Obrov Real Estate, a European real estate platform for the Balkans. Answer questions based on the FAQ below. If the answer is not in the FAQ, say so honestly and refer to info@obrovrealestate.nl. Always answer in the language of the question. Be concise and direct. Never use em-dashes.\n\nFAQ:\n${faqContext}`,
          messages: [{ role: "user", content: vraag }],
        }),
      });
      const data = await res.json();
      setAntwoord(data.content?.find(b => b.type === "text")?.text || "...");
    } catch {
    ...
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: "#FAF8F4", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <header style={{ background: "#fff", borderBottom: `1px solid ${BORDER}`, padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <select value={taal} onChange={(e) => { setTaal(e.target.value); localStorage.setItem("obrov_lang", e.target.value); }} style={{ border: `1px solid ${BORDER}`, borderRadius: 4, padding: "6px 10px", fontSize: 13, fontFamily: "inherit", background: "#fff", cursor: "pointer" }}>
            {Object.entries(TAAL_NAMEN).map(([code, naam]) => <option key={code} value={code}>{naam}</option>)}
          </select>
