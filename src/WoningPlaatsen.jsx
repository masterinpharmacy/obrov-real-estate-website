...
export default function WoningPlaatsen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Maak de woning aan in Supabase (status: concept, betaald: false)
      ...
      // 2. Upload eigen foto's naar Supabase Storage
      ...
      // 3. Start Stripe betaling
      const stripeRes = await fetch("/api/stripe-checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ woningId: woning.id, verkoperEmail: form.verkoper_email }),
      });
      const { url, error: stripeError } = await stripeRes.json();
      if (stripeError || !url) throw new Error(stripeError || "Stripe sessie mislukt");

      // 4. Redirect naar Stripe
    ...
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: CREAM, minHeight: "100vh" }}>
      ...
      <div style={{ background: NAVY, padding: "48px 24px 36px", textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(22px, 4vw, 36px)", color: "#FAF8F4", margin: "0 0 12px", fontWeight: 700 }}>
          Zet uw woning te koop
        </h1>
        <p style={{ color: "#C9D2D0", fontSize: 15, maxWidth: 520, margin: "0 auto 20px" }}>
          Eenmalig €500 plaatsingskosten — inclusief een professionele fotosessie ter plekke in Montenegro.
        </p>
        {/* Stapindicator */}
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Field label="Locatie op kaart — GPS-coördinaten">
              <p style={{ fontSize: 13, color: "#6B6B6B", margin: "0 0 8px" }}>
                Ga naar <a href="https://maps.google.com" target="_blank" rel="noreferrer" style={{ color: GOLD }}>Google Maps</a>, klik op de locatie van uw woning, en kopieer de coördinaten.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <input type="text" placeholder="Breedtegraad (bijv. 42.4304)" value={form.lat} onChange={(e) => set("lat", e.target.value)} style={inputStyle} />
                <input type="text" placeholder="Lengtegraad (bijv. 18.6913)" value={form.lng} onChange={(e) => set("lng", e.target.value)} style={inputStyle} />
              </div>
            </Field>
        ...
        {stap === 4 && (
          <div style={{ textAlign: "center" }}>
            ...
            <h2 style={{ fontFamily: "Georgia, serif", fontSize: 26, color: NAVY, margin: "0 0 16px" }}>Klaar voor plaatsing</h2>
            <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 6, padding: "24px 20px", marginBottom: 28, textAlign: "left" }}>
              <div style={{ fontSize: 13, color: "#6B6B6B", marginBottom: 6 }}>SAMENVATTING</div>
              ...
              <div style={{ borderTop: `1px solid ${BORDER}`, marginTop: 16, paddingTop: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 16, fontWeight: 700, color: NAVY }}>
                  <span>Plaatsingskosten</span>
                  <span>€500</span>
                </div>
                <div style={{ fontSize: 13, color: "#6B6B6B", marginTop: 4 }}>Inclusief professionele fotosessie ter plekke</div>
              </div>
            </div>

            {/* Wat je krijgt */}
            <div style={{ background: "#F2EDE0", borderRadius: 4, padding: "18px 20px", marginBottom: 28, textAlign: "left" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: NAVY, marginBottom: 10 }}>WAT U KRIJGT</div>
              {[
                "Directe plaatsing op obrovrealestate.nl (na betaling)",
                "Uw woning zichtbaar voor Nederlandse, Engelse en Duitse investeerders",
                "Professionele fotosessie ter plekke in Montenegro",
                "Foto's worden vervangen door professionele beelden na de sessie",
                "Uw contactgegevens zichtbaar op de detailpagina",
              ].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "flex-start" }}>
                  <span style={{ color: GOLD, fontWeight: 700, flexShrink: 0 }}>✓</span>
                  <span style={{ fontSize: 14, color: "#3D3D3D" }}>{item}</span>
                </div>
              ))}
            </div>

            <p style={{ fontSize: 13, color: "#6B6B6B", marginBottom: 20 }}>
              U wordt doorgestuurd naar Stripe voor de veilige betaling. Na betaling is uw woning direct zichtbaar op de website.
            </p>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 32, justifyContent: stap > 0 ? "space-between" : "flex-end" }}>
          ...
          {stap < 4 ? (
            ...
            <button
              ...
              disabled={uploading}
              style={{ background: GOLD, color: "#fff", border: "none", padding: "15px 32px", borderRadius: 3, fontWeight: 700, fontSize: 15, cursor: uploading ? "default" : "pointer", opacity: uploading ? 0.7 : 1 }}
            >
              {uploading ? "Bezig met uploaden..." : "Betalen & plaatsen — €500 →"}
            </button>
          )}
        </div>
      </div>
