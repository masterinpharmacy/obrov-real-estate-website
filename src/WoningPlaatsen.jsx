...
export default function WoningPlaatsen() {
  ...
  // Auth check — doorsturen naar login als niet ingelogd
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) {
        setGebruiker(data.session.user);
      } else {
        navigate("/auth?modus=inloggen&redirect=/woning-plaatsen");
      }
      setAuthLoading(false);
    });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch("/api/auth-notificatie", {
        ...
        body: JSON.stringify({
          type: "plaatsing",
          naam: form.verkoper_naam,
          email: form.verkoper_email,
          woning: { type: form.type, stad: form.stad, vraagprijs: form.vraagprijs },
        }),
      }).catch(() => {});

      // Direct doorsturen naar succespagina
