...
const INIT_FORM = {
  ...
  // Kenmerken
  tuin: false, zwembad: false, garage: false, zeezicht: false, bergzicht: false,
  ...
  gemeubileerd: false, nieuwbouw: false, renovatie_nodig: false,
  // Belasting
  belasting_per_jaar: "",
  // AI omschrijving
  ai_info_dump: "", omschrijving_nl: "", omschrijving_en: "", ai_generating: false,
  // Foto's
...
export default function WoningPlaatsen() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) setGebruiker(data.session.user);
  ...
  const handleFotos = (e) => {
    ...
    set("_fotoFiles", files);
    setFotoPreview(files.map((f) => URL.createObjectURL(f)));
  };

  const handlePlattegrond = (e) => {
    const files = Array.from(e.target.files);
    set("_plattegrondFiles", files);
    setPlattegrondPreview(files.map((f) => URL.createObjectURL(f)));
  };
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `Je bent een vastgoedexpert die professionele woningbeschrijvingen schrijft voor Obrov Real Estate, een Europees platform voor Balkan-vastgoed. Schrijf aantrekkelijke, informatieve beschrijvingen op basis van de gegeven informatie. Verrijk de beschrijving met context over de locatie: natuur, inwoners, demografie, prijzen in de buurt, voorzieningen (treinstations, supermarkten, scholen, luchthavens), en toekomstige plannen voor de wijk/stad/regio. Gebruik geen em-dashes (--). Schrijf in een professionele maar toegankelijke toon. Geef de beschrijving in JSON: {"nl": "...", "en": "..."} en niets anders.`,
          messages: [{ role: "user", content: `Woning info: ${form.ai_info_dump}\n\nType: ${form.type}, Stad: ${form.stad}, Regio: ${form.regio || ""}, Vraagprijs: €${form.vraagprijs || ""}, Oppervlakte: ${form.oppervlakte_m2 || ""}m², Slaapkamers: ${form.slaapkamers || ""}, Bouwjaar: ${form.bouwjaar || ""}` }],
        }),
      });
      ...
      set("omschrijving_nl", parsed.nl || "");
      set("omschrijving_en", parsed.en || "");
    } catch {
      setError("AI genereren mislukt. Probeer opnieuw.");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: woning, error: dbError } = await supabase.from("woningen").insert([{
        ...
        bergzicht: form.bergzicht, water: form.water, elektriciteit: form.elektriciteit,
    ...
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: CREAM, minHeight: "100vh" }}>
      ...
      <div className="hero-padding" style={{ background: NAVY, padding: "36px 24px 28px", textAlign: "center" }}>
        ...
        <div style={{ display: "inline-block", background: "rgba(46,125,50,0.25)", border: "1px solid rgba(46,125,50,0.5)", color: "#A5D6A7", fontSize: 13, fontWeight: 700, padding: "5px 16px", borderRadius: 20, margin: "8px 0 14px" }}>
          Gratis plaatsen t/m 31 oktober 2026, daarna €195 eenmalig (particulier)
        </div>
      ...
      <div className="form-padding" style={{ maxWidth: 720, margin: "0 auto", padding: "36px 24px 80px" }}>
        ...
        {stap === 1 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {(form.omschrijving_nl || form.omschrijving_en) && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <Field label="Omschrijving Engels (gegenereerd door AI, aanpasbaar)">
              ...
              {plattegrondPreview.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8, marginTop: 12 }}>
                  {plattegrondPreview.map((url, i) => (
                    url.startsWith("blob:") && <img key={i} src={url} alt="" style={{ width: "100%", height: 110, objectFit: "contain", borderRadius: 3, border: `1px solid ${BORDER}`, background: "#f5f5f5" }} />
                  ))}
                  {form._plattegrondFiles.length > 0 && (
                    <div style={{ fontSize: 13, color: "#2E7D32", padding: "8px 0" }}>
                      {form._plattegrondFiles.length} bestand(en) geselecteerd
                    </div>
                  )}
                </div>
              )}
        ...
        {stap === 5 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 20 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: NAVY, marginBottom: 14 }}>Uw contactgegevens</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {stap === 6 && (
          <div>
            ...
            <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 6, padding: "20px", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12, color: "#6B6B6B", marginTop: 4 }}>Gratis t/m 31 oktober 2026. Daarna EUR 195 eenmalig (particulier).</div>
            </div>
