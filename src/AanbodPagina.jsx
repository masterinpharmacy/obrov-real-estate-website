...
${metCoord.map((w) => {
  ...
  return `L.marker([${w.lat},${w.lng}],{icon:L.divIcon({className:'',html:'<div style="background:#0B2A52;color:#fff;font-size:11px;font-weight:700;padding:4px 9px;border-radius:12px;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.3);cursor:pointer">€${Math.round(w.vraagprijs/1000)}k</div>',iconAnchor:[25,16]})}).addTo(map).bindPopup(\`${popupHtml.replace(/`/g, "'")}\`,{maxWidth:220,minWidth:220});`;
}).join("\n")}
