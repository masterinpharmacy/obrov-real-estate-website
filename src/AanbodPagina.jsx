...
${metCoord.map((w) => {
  ...
  return `L.marker([${w.lat},${w.lng}],{icon:L.divIcon({className:'',html:'<div style="background:#fff;color:#0B2A52;font-size:11px;font-weight:700;padding:4px 10px;border-radius:12px;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,0.25);cursor:pointer;border:1.5px solid #0B2A52">€${Math.round(w.vraagprijs/1000)}k</div>',iconAnchor:[25,16]})}).addTo(map).bindPopup(\`${popupHtml.replace(/`/g, "'")}\`,{maxWidth:220,minWidth:220});`;
}).join("\n")}
