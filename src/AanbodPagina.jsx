  ...
  return (
    <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "16px 18px", position: "sticky", top: 130 }}>
      ...
      <Section title="KENMERKEN">
        ...
        <CheckItem label="Rivieruitzicht" filterKey="rivierzicht" />
...
const FILTERS_DEFAULT = {
  ...
  zeezicht: false, bergzicht: false, rivierzicht: false, zwembad: false, tuin: false,
...
export default function AanbodPagina() {
  ...
  const gefilterd = woningen.filter((w) => {
    ...
    if (filters.rivierzicht && !w.rivierzicht) return false;
  ...
  return (
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: "#FAF8F4", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          <Link to="/"><img src="/logo.png" alt="Obrov Real Estate" style={{ height: 40, width: "auto", flexShrink: 0 }} /></Link>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "nowrap", overflow: "hidden" }}>
            <Link to="/" style={{ fontSize: 13, color: NAVY, textDecoration: "none", fontWeight: 500, whiteSpace: "nowrap" }}>Home</Link>
            <button onClick={() => setShowFavs(true)} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: `1.5px solid ${BORDER}`, borderRadius: 20, padding: "6px 10px", cursor: "pointer", fontSize: 13, color: NAVY, whiteSpace: "nowrap", flexShrink: 0 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill={favs.size > 0 ? "#e53935" : "none"} stroke={favs.size > 0 ? "#e53935" : NAVY} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
              Favorieten {favs.size > 0 && <span style={{ background: "#e53935", color: "#fff", borderRadius: "50%", width: 16, height: 16, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>{favs.size}</span>}
            </button>
            <Link to="/favorieten" style={{ fontSize: 13, color: NAVY, textDecoration: "none", fontWeight: 500, whiteSpace: "nowrap" }}>Mijn account</Link>
            <Link to="/woning-plaatsen" style={{ background: GOLD, color: "#fff", padding: "7px 12px", borderRadius: 20, fontSize: 12.5, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>
              + Plaatsen
            </Link>
          </div>
        </div>
