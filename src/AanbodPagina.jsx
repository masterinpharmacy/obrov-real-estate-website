...
export default function AanbodPagina() {
  ...
  useEffect(() => {
    // Toon demo-woningen direct — geen wachten op Supabase
    setWoningen(DEMO_WONINGEN);
    setLoading(false);

    // Probeer Supabase op de achtergrond (met timeout van 4 seconden)
    ...
    async function laadVanDatabase() {
      try {
        ...
        if (!error && data && data.length > 0) {
          setWoningen(data); // Vervang demo's met echte data als beschikbaar
        }
      } catch {
        // Stil falen — demo-woningen blijven zichtbaar
      } finally {
    ...
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: "#FAF8F4", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        .filter-popup{position:absolute;top:calc(100% + 8px);left:0;background:#fff;border:1px solid ${BORDER};border-radius:8px;padding:18px 18px 14px;min-width:240px;z-index:110;box-shadow:0 4px 20px rgba(0,0,0,0.12)}
        .filter-bar{position:sticky;top:66px;z-index:45;background:#fff;border-bottom:1px solid ${BORDER};box-shadow:0 2px 8px rgba(0,0,0,0.06)}
      `}</style>
      ...
      {/* DEMO BANNER */}
      {isDemo && (
        <div style={{ background: "#FFF8E7", borderBottom: "1.5px solid #F0C040", padding: "10px 24px" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto", fontSize: 13.5, color: "#7A5C00", display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 16 }}>⚡</span>
            <span><strong>Demo-modus</strong> — Dit zijn voorbeeldwoningen. Zodra Supabase is geconfigureerd worden echte woningen getoond. <a href="/woning-plaatsen" style={{ color: "#AC9362", fontWeight: 700 }}>Woning plaatsen →</a></span>
          </div>
        </div>
      )}
