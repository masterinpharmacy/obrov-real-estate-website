    ...
    <div style={{ fontFamily: "Helvetica, Arial, sans-serif", background: "#FAF8F4", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        .filter-popup{position:absolute;top:calc(100% + 6px);left:0;background:#fff;border:1px solid ${BORDER};border-radius:8px;padding:18px 18px 14px;min-width:240px;z-index:200;box-shadow:0 4px 20px rgba(0,0,0,0.15)}
      `}</style>
      ...
      {/* STICKY WRAPPER — zoekbalk + filterbalk kleven samen onder de navbar */}
      <div style={{ position: "sticky", top: 66, zIndex: 100 }}>

        {/* ZOEKBALK */}
        <div style={{ background: NAVY, padding: "16px 24px 0" }}>
            ...
            <div style={{ display: "flex", gap: 0, background: "#fff", borderRadius: "8px 8px 0 0", overflow: "hidden", maxWidth: 860 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", flex: 2, padding: "0 14px", borderRight: `1px solid ${BORDER}` }}>
                ...
                <input
                  ...
                  style={{ flex: 1, border: "none", outline: "none", fontSize: 14.5, padding: "13px 0", fontFamily: "inherit", background: "transparent" }}
                />
              ...
              {/* Km-straal */}
              <div style={{ display: "flex", alignItems: "center", padding: "0 14px", borderRight: `1px solid ${BORDER}`, minWidth: 150 }}>
                <select
                  value={zoekKm}
                  onChange={(e) => setZoekKm(e.target.value)}
                  disabled={!zoekCoords}
                  style={{ border: "none", outline: "none", fontSize: 14, fontFamily: "inherit", background: "transparent", color: zoekCoords ? "#1A1A1A" : "#aaa", cursor: zoekCoords ? "pointer" : "default", padding: "13px 0", minWidth: 120 }}
                >
        ...
        {/* FILTER CHIPS BALK */}
        <div style={{ background: "#fff", borderBottom: `1px solid ${BORDER}`, boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto", padding: "8px 24px", display: "flex", gap: 8, overflowX: "auto", alignItems: "center" }}>
          {/* Koop/Huur chip */}
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: weergave === "kaart" ? "16px 24px 0" : "24px 24px 60px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "80px 0", color: "#6B6B6B" }}>
            <div style={{ fontSize: 16 }}>Woningen laden...</div>
          </div>
        ) : gefilterd.length === 0 ? (
